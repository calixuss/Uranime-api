...
exports.getRequestTypeById = function(req, res){
	...
	Models.ScrapeType.find(id).success(function(type){
		res.send(type);
	});
};	
...
exports.getSiteById = function(req, res){
	...
	Models.Site.find(id).success(function(site){
		res.send(site);
	});
};
...
exports.getRequests = function(req, res){
	var Models = req.app.get("models");
    var includeQuery = [
        {model: Models.RequestInfo, as:'RequestInfo'},
        {model: Models.RequestAttribute, as: 'RequestAttributes'}
    ];
    // Fetch requests
    // Fetch associated attribute types
    Models.Request.findAll({include:includeQuery}).success(function(requests){
        res.send(requests);
    });
