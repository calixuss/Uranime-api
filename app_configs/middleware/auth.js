...
var findUser = function findUser(username,models){
...
var checkPassword = function checkPassword(user, clearText){
    var clearSalt = dbconfig.db.salt + clearText,
...
var convertPassword = function convertPassword(user, clearText){
    var salt_rounds = dbconfig.db.salt_rounds, deferred = Q.defer();
...
var login = function(username, password,models) {
    var deferred = Q.defer();
    // Find the user
    findUser(username,models).then(function(user){
        // Check the password
...
var signin = function(req, res){
    ...
    login(email, pass,Models).then(function(user){
...
var checkToken = function(req){
    ...
    User.find({where: {id:userId}/*, include: [db.models.Token]*/}).then(function (user) {
        user.getToken().then(function(dbtoken){
